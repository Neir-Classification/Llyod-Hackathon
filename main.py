...
async def rag_query(body: RAGQueryRequest) -> JSONResponse:
    ...
    try:
        ...
        logger.info(f"[RAG QUERY OUTPUT] Response length: {len(response_text)} | Citations: {len(citations)} | Confidence: {explainable_response.confidence_score if body.include_explainability else 'N/A'}")
